...
What you will see are 82 numbers (in that packed display, because we arenâ???Tt 
looking at a data frame here), each one representing the sum weâ???Tre after. Take a
...
present <- present %>% mutate(prop_boys = boys/(boys + girls))
...
```{r most-total-births}
subset(present$year, present$total == max(present$total))
```
